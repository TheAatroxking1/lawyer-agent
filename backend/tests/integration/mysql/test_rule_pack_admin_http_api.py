...
def test_rule_pack_admin_http_full_flow_over_mysql_redis(
    ...
    prefix = f"lawyer-test-pack-admin-http:{uuid4().hex}:"
    settings = Settings(
        environment="test",
        secret_key="h" * 32,
    ...
    with client:
        ...
        for index in (1, 2):
            created = client.post(
                ...
                headers={
                    ...
                    "Idempotency-Key": f"pack-admin-tenant-{index}",
                },
        ...
        pack_v1_id = pack_v1["id"]

        # 2. Add a risk-phrase rule to v1.
        rule = client.post(
            f"{base_a}/rule-packs/{pack_v1_id}/rules",
            headers=headers_a,
            json={
        ...
        assert rule.json()["trigger_kind"] == "risk_phrase"

        # 3. Add a broken-regex rule -> domain rejection (422).
        bad = client.post(
            f"{base_a}/rule-packs/{pack_v1_id}/rules",
            headers=headers_a,
            json={
        ...
        assert bad.json()["code"] == "rule_pack_admin_invalid_request"

        # 4. Create a second same-name version (v2, inactive).
        pack2 = client.post(
            f"{base_a}/rule-packs",
            headers=headers_a,
            json={"name": "租赁合同"},
        ...
        # 5. Activate v1 -> unique active.
        activated = client.post(
            f"{base_a}/rule-packs/{pack_v1_id}/activate",
            headers=headers_a,
        )
        assert activated.status_code == 200, activated.text
        assert activated.json()["active"] is True

        listed = client.get(f"{base_a}/rule-packs", headers=headers_a)
        assert listed.status_code == 200, listed.text
        active_packs = [pack for pack in listed.json() if pack["active"]]
        assert len(active_packs) == 1 and active_packs[0]["id"] == pack_v1_id

        # 6. Drive the rule through a real check: matter + docx + run.
        matter = client.post(
            f"{base_a}/matters",
            headers=headers_a,
        ...
        assert ran.json()[0]["risk_level"] == "high"

        # 7. Disable the rule -> the pack has no enabled rules, so a check run
        #    is refused (empty would be mistaken for "no risk").
        disabled = client.patch(
            f"{base_a}/rule-packs/{pack_v1_id}/rules/{rule_id}",
            headers=headers_a,
            json={"enabled": False},
        )
        assert disabled.status_code == 204, disabled.text
        ran_after = client.post(
            f"{base_a}/documents/{document_id}/risk-checks",
            headers=headers_a,
            json={
                "provisions": [
        ...
        assert ran_after.status_code == 409
        assert ran_after.json()["code"] == "rule_check_unavailable"

        # 8. Tenant B cannot manage tenant A's packs.
        ...
        stolen_activate = client.post(
            f"{base_b}/rule-packs/{pack_v1_id}/activate",
            headers={"Authorization": f"Bearer {token_b}"},
        )
        ...
        empty = client.get(f"{base_b}/rule-packs", headers={"Authorization": f"Bearer {token_b}"})
