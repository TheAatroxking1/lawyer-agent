...
def test_document_review_http_full_flow_over_mysql_redis(
    ...
    prefix = f"lawyer-test-review-http:{uuid4().hex}:"
    settings = Settings(
        environment="test",
        secret_key="h" * 32,
    ...
    with client:
        ...
        for index in (1, 2):
            created = client.post(
                ...
                headers={
                    ...
                    "Idempotency-Key": f"review-http-tenant-{index}",
                },
                json={"name": f"复核租户{index}", "tenant_type": "law_firm"},
            )
        ...
        # Build an accepted document version through the real upload endpoint.
        matter = client.post(
            f"{base_a}/matters",
        ...
        # 1. Submit for review (None -> PENDING_REVIEW).
        ...
        # 2. Request changes with a mandatory reason.
        changed = client.post(
            review_url,
            headers=headers_a,
            json={
        ...
        assert changed.json()["review_status"] == "changes_requested"
        assert changed.json()["review_reason"] == "需补充授权签署页与日期"

        # 3. Requesting changes without a reason is rejected (422).
        ...
        # 4. Re-submit after changes.
        resubmitted = client.post(review_url, headers=headers_a, json={"decision": "submit"})
        assert resubmitted.status_code == 200, resubmitted.text
        assert resubmitted.json()["review_status"] == "pending_review"
