...
def test_document_version_read_back_over_real_mysql(migrated_mysql_url: URL) -> None:
    ...
    prefix = f"lawyer-test-ver-read:{uuid4().hex}:"
    settings = Settings(
        environment="test",
        secret_key="h" * 32,
    ...
    with client:
        ...
        for index in (1, 2):
            created = client.post(
                ...
                headers={
                    ...
                    "Idempotency-Key": f"ver-read-tenant-{index}",
                },
                json={"name": f"版本租户{index}", "tenant_type": "law_firm"},
            )
        ...
        made = client.post(
            f"{base_a}/matters",
            headers=headers_a,
            json={"title": "审查文档", "kind": "contract_review"},
        )
        ...
        version_no = uploaded.json()["version_no"]
        version_url = f"{base_a}/documents/{document_id}/versions/{version_no}"

        # 1. Read back upload/review state right after register.
        first = client.get(version_url, headers=headers_a)
        ...
        assert body["upload_status"] == "accepted"
        assert body["review_status"] is None  # not yet submitted (== draft)
        assert body["review_reason"] is None

        # 2. submit -> GET shows pending_review.
        submitted = client.post(
            f"{version_url}/review",
            headers=headers_a,
            json={"decision": "submit", "reason": None},
        )
        assert submitted.status_code == 200, submitted.text
        after_submit = client.get(version_url, headers=headers_a)
        assert after_submit.status_code == 200, after_submit.text
        assert after_submit.json()["review_status"] == "pending_review"

        # 3. request_changes with reason -> GET shows it and the reason.
        changed = client.post(
            f"{version_url}/review",
            headers=headers_a,
            json={"decision": "request_changes", "reason": "补充授权页与日期"},
        )
        assert changed.status_code == 200, changed.text
        after_change = client.get(version_url, headers=headers_a)
        assert after_change.status_code == 200, after_change.text
        assert after_change.json()["review_status"] == "changes_requested"
        assert after_change.json()["review_reason"] == "补充授权页与日期"

        # 4. resubmit then approve -> GET shows approved and reason cleared.
        ...
        final = client.get(version_url, headers=headers_a)
        assert final.status_code == 200, final.text
        assert final.json()["review_status"] == "approved"
        assert final.json()["review_reason"] is None

        # 5. Unknown version / document -> 404.
        missing = client.get(
            f"{base_a}/documents/{document_id}/versions/999", headers=headers_a
        )
        assert missing.status_code == 404
        assert missing.json()["code"] == "document_review_not_found"

        # 6. Cross-tenant isolation (path and resource layers).
        ...
        stolen_path = client.get(version_url, headers={"Authorization": f"Bearer {token_b}"})
        ...
        stolen_resource = client.get(
            f"{base_b}/documents/{document_id}/versions/{version_no}",
            headers={"Authorization": f"Bearer {token_b}"},
        )
        assert stolen_resource.status_code == 404
        assert stolen_resource.json()["code"] == "document_review_not_found"
