...
def test_matter_description_read_back_over_real_mysql(migrated_mysql_url: URL) -> None:
    ...
    prefix = f"lawyer-test-desc:{uuid4().hex}:"
    settings = Settings(
        environment="test",
        secret_key="h" * 32,
    ...
    with client:
        ...
        for index in (1, 2):
            created = client.post(
                ...
                headers={
                    ...
                    "Idempotency-Key": f"desc-tenant-create-{index}",
                },
                json={"name": f"描述租户{index}", "tenant_type": "law_firm"},
            )
        ...
        logged_in = client.post(
            ...
            json={
                "kind": "username",
                "identifier": "matter-desc-readback",
                "password": _PASSWORD,
            },
        )
        ...
        # 1. Create response includes description.
        made = client.post(
            f"{base_a}/matters",
            headers=headers_a,
            json={
        ...
        assert made.json()["description"] == "初版描述"
        matter_id = made.json()["id"]
        url = f"{base_a}/matters/{matter_id}"

        # 2. GET single reads description back.
        got = client.get(url, headers=headers_a)
        assert got.status_code == 200, got.text
        assert got.json()["description"] == "初版描述"

        # 3. PATCH title+description read back updated value.
        ...
        assert patched.json()["matter"]["description"] == "补充租金约定"
        assert patched.json()["matter"]["title"] == "房屋租赁纠纷"
        assert patched.json()["matter"]["version"] == 2

        # 4. Description-only edit preserves title; read-back reflects it.
        desc_only = client.patch(url, headers=headers_a, json={"description": "描述二版"})
        assert desc_only.status_code == 200, desc_only.text
        assert desc_only.json()["matter"]["title"] == "房屋租赁纠纷"
        assert desc_only.json()["matter"]["description"] == "描述二版"

        # 5. Explicit empty description clears; read-back is null.
        cleared = client.patch(url, headers=headers_a, json={"description": ""})
        assert cleared.status_code == 200, cleared.text
        assert cleared.json()["matter"]["description"] is None
        got_after = client.get(url, headers=headers_a)
        assert got_after.json()["description"] is None

        # 6. List page items include description.
        listed = client.get(f"{base_a}/matters", headers=headers_a)
        assert listed.status_code == 200, listed.text
        item = next(entry for entry in listed.json()["items"] if entry["id"] == matter_id)
        assert item["description"] is None

        # 7. Re-set a description, then list shows it; owner assignment keeps it.
        reset = client.patch(url, headers=headers_a, json={"description": "定稿描述"})
        assert reset.status_code == 200, reset.text
        listed_after = client.get(f"{base_a}/matters", headers=headers_a)
        item_after = next(
            entry for entry in listed_after.json()["items"] if entry["id"] == matter_id
        )
        assert item_after["description"] == "定稿描述"

        # 8. Cross-tenant: B cannot read A's description via path or list.
        ...
        stolen = client.get(url, headers={"Authorization": f"Bearer {token_b}"})
        ...
        assert all(entry["id"] != matter_id for entry in own.json()["items"])
