...
async def _seed_member(mysql_url: URL, tenant_id: str) -> str:
    user_id = new_uuid7()
        ...
        async with engine.begin() as connection:
            await connection.execute(
                text(
                    ...
                    "VALUES (:id,'active','Clear 合成成员',1,1)"
                ),
                {"id": user_id.bytes},
            )
            await connection.execute(
                text(
                    ...
                    "VALUES (:id,:tenant_id,:user_id,'internal','active',"
                    "UTC_TIMESTAMP(6),1,1)"
                ),
                {
...
def test_matter_owner_clear_over_real_mysql(migrated_mysql_url: URL) -> None:
    ...
    prefix = f"lawyer-test-owner-clear:{uuid4().hex}:"
    settings = Settings(
        environment="test",
        secret_key="h" * 32,
    ...
    with client:
        registered = client.post(
            ...
            json={
                "username": "owner-clear-user",
                "password": _PASSWORD,
                "display_name": "负责人解除用户",
            },
        )
        ...
        for index in (1, 2):
            created = client.post(
                ...
                headers={
                    ...
                    "Idempotency-Key": f"owner-clear-tenant-{index}",
                },
                json={"name": f"解除租户{index}", "tenant_type": "law_firm"},
            )
        ...
        member_a = asyncio.run(
            _seed_member(migrated_mysql_url, tenant_ids[0])
        )
        ...
        made = client.post(
            f"{base_a}/matters",
            headers=headers_a,
            json={"title": "负责人解除案件", "kind": "litigation"},
        )
        ...
        # 1. Assign an owner first, then clear it.
        assigned = client.put(
            owner_url, headers=headers_a, json={"owner_membership_id": member_a}
        )
        assert assigned.status_code == 200, assigned.text
        assert assigned.json()["matter"]["owner_membership_id"] == member_a
        assert assigned.json()["matter"]["version"] == 2

        cleared = client.delete(owner_url, headers=headers_a)
        assert cleared.status_code == 200, cleared.text
        assert cleared.json()["matter"]["owner_membership_id"] is None
        assert cleared.json()["matter"]["version"] == 3
        stored = asyncio.run(_matter_owner(migrated_mysql_url, matter_id))
        assert stored is None

        # 2. GET reflects cleared owner; If-Match stale -> 409.
        got = client.get(f"{base_a}/matters/{matter_id}", headers=headers_a)
        assert got.status_code == 200, got.text
        assert got.json()["owner_membership_id"] is None
        stale = client.delete(
            owner_url, headers={**headers_a, "If-Match": '"2"'}
        )
        assert stale.status_code == 409
        assert stale.json()["code"] == "matter_document_conflict"

        # 3. Missing matter -> 404.
        missing = client.delete(
            f"{base_a}/matters/{new_uuid7()}/owner", headers=headers_a
        )
        assert missing.status_code == 404
        assert missing.json()["code"] == "matter_document_not_found"

        # 4. Cross-tenant isolation (path + resource layers).
        ...
        stolen_path = client.delete(
            owner_url, headers={"Authorization": f"Bearer {token_b}"}
        )
        ...
        # 5. Audit rows: assigned then unassigned, visible to A only.
        ...
        reasons = {event["reason_code"] for event in owner_events}
        assert reasons == {"assigned", "unassigned"}
