...
def test_tenant_matter_list_pagination_over_real_mysql(migrated_mysql_url: URL) -> None:
    ...
    prefix = f"lawyer-test-matter-list:{uuid4().hex}:"
    settings = Settings(
        environment="test",
        secret_key="h" * 32,
    ...
    with client:
        ...
        for index in (1, 2):
            created = client.post(
                ...
                headers={
                    ...
                    "Idempotency-Key": f"matter-list-tenant-{index}",
                },
                json={"name": f"案件租户{index}", "tenant_type": "law_firm"},
            )
        ...
        # Tenant A: three matters.
        created_ids: list[str] = []
        for title in ("租赁纠纷", "借款纠纷", "股权纠纷"):
            made = client.post(
                f"{base_a}/matters",
                headers=headers_a,
                json={"title": title, "kind": "litigation"},
            )
            assert made.status_code == 201, made.text
            created_ids.append(made.json()["id"])

        # Tenant B: one matter (must never leak into A's pages).
        ...
        made_b = client.post(
            ...
            json={"title": "乙租户案件", "kind": "other"},
        )
        assert made_b.status_code == 201, made_b.text

        # 1. First page of limit=2 returns two items plus a cursor.
        page1 = client.get(f"{base_a}/matters?limit=2", headers=headers_a)
        ...
        # 2. Second page returns the remaining one and no cursor.
        page2 = client.get(
            f"{base_a}/matters?limit=2&before_id={body1['next_before_id']}",
            headers=headers_a,
        )
        ...
        # 3. Invalid limit bounds -> 422.
        low = client.get(f"{base_a}/matters?limit=0", headers=headers_a)
        assert low.status_code == 422
        high = client.get(f"{base_a}/matters?limit=101", headers=headers_a)
        assert high.status_code == 422

        # 4. Unknown/foreign cursor id -> 404 (no cross-tenant enumeration).
        fake_cursor = "00000000-0000-7000-8000-000000000099"
        unknown = client.get(
            f"{base_a}/matters?limit=2&before_id={fake_cursor}", headers=headers_a
        )
        assert unknown.status_code == 404
        assert unknown.json()["code"] == "matter_document_not_found"

        # 5. Tenant B path-layer call to A's listing -> 404; B's own listing only
        #    contains its own matter and never A's ids.
        stolen = client.get(
            f"{base_a}/matters?limit=10", headers={"Authorization": f"Bearer {token_b}"}
        )
        assert stolen.status_code == 404
        assert stolen.json()["code"] == "tenant_resource_not_found"
        own = client.get(
            f"{base_b}/matters?limit=10",
            headers={"Authorization": f"Bearer {token_b}"},
        )
        assert own.status_code == 200, own.text
        own_items = own.json()["items"]
        assert {item["id"] for item in own_items} == {made_b.json()["id"]}
