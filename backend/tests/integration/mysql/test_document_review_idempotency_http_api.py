...
async def _review_version_status(mysql_url: URL, version_id: str) -> str | None:
    engine = create_async_engine(mysql_url)
    try:
        async with engine.connect() as connection:
            value = await connection.scalar(
                text(
                    "SELECT review_status FROM tenant_document_versions "
                    "WHERE id = :id"
                ).bindparams(id=UUID(version_id).bytes)
            )
            return value
    finally:
        await engine.dispose()
...
def test_document_review_is_idempotent_over_real_mysql(migrated_mysql_url: URL) -> None:
    ...
    prefix = f"lawyer-test-review-idem:{uuid4().hex}:"
    settings = Settings(
        environment="test",
        secret_key="h" * 32,
    ...
    with client:
        ...
        created = client.post(
            ...
            headers={
                ...
                "Idempotency-Key": "review-idem-tenant-1",
            },
            json={"name": "复核幂等租户", "tenant_type": "law_firm"},
        )
        ...
        review_url = f"{base}/documents/{document_id}/versions/1/review"
        review_headers = {
            "Authorization": f"Bearer {token}",
            "Idempotency-Key": "document-review-0000000001",
        }

        # 1. First submit with a key succeeds (pending_review).
        first = client.post(review_url, headers=review_headers, json={"decision": "submit"})
        assert first.status_code == 200, first.text
        assert first.json()["review_status"] == "pending_review"
        assert (
            asyncio.run(_review_version_status(migrated_mysql_url, version_id))
            == "pending_review"
        )

        # 2. Replay same key and decision returns the same projection without a
        #    second transition or an idempotency conflict.
        replay = client.post(review_url, headers=review_headers, json={"decision": "submit"})
        assert replay.status_code == 200, replay.text
        assert replay.json()["review_status"] == "pending_review"

        # 3. Same key with a different decision conflicts.
        ...
        # 4. A different key can advance the same version to approved.
        approve_headers = {
            "Authorization": f"Bearer {token}",
            "Idempotency-Key": "document-review-0000000002",
        }
        approved = client.post(
            review_url, headers=approve_headers, json={"decision": "approve"}
        )
        ...
        # 5. Replaying the original submit key still returns success (completed
        #    result read back), even though the version is now terminal.
        replay_after = client.post(
            review_url, headers=review_headers, json={"decision": "submit"}
        )
        assert replay_after.status_code == 200, replay_after.text

        # 6. A fresh key on a terminal version hits the state machine conflict.
        terminal = client.post(
            review_url,
            headers={
                "Authorization": f"Bearer {token}",
                "Idempotency-Key": "document-review-0000000003",
            },
            json={"decision": "reject", "reason": "终态复核"},
        )
        assert terminal.status_code == 409
        assert terminal.json()["code"] == "document_review_conflict"

        # 7. No-key requests still behave exactly as before (409 on terminal).
        no_key = client.post(
            review_url,
            headers={"Authorization": f"Bearer {token}"},
            json={"decision": "request_changes", "reason": "再次复核"},
        )
        assert no_key.status_code == 409
        assert no_key.json()["code"] == "document_review_conflict"
