...
- 阶段 2 增补“Rule Pack 管理 HTTP API”非模型切片已完成（2026-09-05 计划）：`/api/v1/tenants/{tenant_id}/rule-packs` 下创建（同名版本自动 +1、inactive）/列表/向 Pack 添加规则（正则白名单、枚举校验）/启停规则/激活唯一 active（事务清理其它）；管理纯函数与租户范围仓储写方法，MySQL 集成覆盖版本推进与唯一激活，全栈 HTTP 覆盖 建包→加规则→坏正则 422→v2→激活→联动 Rule Check 命中→停用后 409 拒绝空跑→跨租户双层 404；模板规则库、permission code 化、审计登记仍为延后项。
