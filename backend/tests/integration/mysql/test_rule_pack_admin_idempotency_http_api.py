...
async def _pack_count(mysql_url: URL, tenant_id: UUID) -> int:
    engine = create_async_engine(mysql_url)
    try:
        async with engine.connect() as connection:
            value = await connection.scalar(
                text(
                    "SELECT COUNT(*) FROM tenant_rule_packs WHERE tenant_id = :t"
...
async def _rule_count(mysql_url: URL, tenant_id: UUID) -> int:
    engine = create_async_engine(mysql_url)
    try:
        async with engine.connect() as connection:
            value = await connection.scalar(
                text(
                    "SELECT COUNT(*) FROM tenant_rule_pack_rules WHERE tenant_id = :t"
                ).bindparams(t=tenant_id.bytes)
            )
            return int(value or 0)
    finally:
...
def test_rule_pack_admin_creation_is_idempotent_over_real_mysql(
    ...
    prefix = f"lawyer-test-pack-idem:{uuid4().hex}:"
    settings = Settings(
        environment="test",
        secret_key="h" * 32,
    ...
    with client:
        registered = client.post(
            ...
            json={
                "username": "pack-idem-owner",
                "password": _PASSWORD,
                "display_name": "规则包幂等用户",
            },
        )
        ...
        created = client.post(
            ...
            headers={
                ...
                "Idempotency-Key": "pack-idem-tenant-1",
            },
            json={"name": "规则包幂等租户", "tenant_type": "law_firm"},
        )
        ...
        # --- Create pack idempotently. ---
        pack_body = {"name": "租赁合同"}
        pack_key = {
            "Idempotency-Key": "pack-create-000000000001",
            "Authorization": f"Bearer {token}",
        }
        first = client.post(f"{base}/rule-packs", headers=pack_key, json=pack_body)
        assert first.status_code == 201, first.text
        first_pack = first.json()
        replay = client.post(f"{base}/rule-packs", headers=pack_key, json=pack_body)
        assert replay.status_code == 201, replay.text
        assert replay.json()["id"] == first_pack["id"]
        assert (
            asyncio.run(_pack_count(migrated_mysql_url, UUID(tenant_id))) == 1
        )
        ...
        rule_key = {
            "Idempotency-Key": "rule-add-00000000001",
            "Authorization": f"Bearer {token}",
        }
        rule_first = client.post(
            f"{base}/rule-packs/{pack_id}/rules", headers=rule_key, json=rule_body
        )
        assert rule_first.status_code == 201, rule_first.text
        first_rule = rule_first.json()
        rule_replay = client.post(
            f"{base}/rule-packs/{pack_id}/rules", headers=rule_key, json=rule_body
        )
        assert rule_replay.status_code == 201, rule_replay.text
        assert rule_replay.json()["id"] == first_rule["id"]
        assert asyncio.run(_rule_count(migrated_mysql_url, UUID(tenant_id))) == 1
        rule_conflict = client.post(
            f"{base}/rule-packs/{pack_id}/rules",
            headers=rule_key,
            json={**rule_body, "label": "不同标签"},
        )
        assert rule_conflict.status_code == 409
        assert rule_conflict.json()["code"] == "idempotency_conflict"

        # A different key adds an independent rule.
        second_rule = client.post(
            f"{base}/rule-packs/{pack_id}/rules",
            headers={
                "Idempotency-Key": "rule-add-00000000002",
                "Authorization": f"Bearer {token}",
            },
            json=rule_body,
        )
        assert second_rule.status_code == 201, second_rule.text
        assert second_rule.json()["id"] != first_rule["id"]
        assert asyncio.run(_rule_count(migrated_mysql_url, UUID(tenant_id))) == 2
