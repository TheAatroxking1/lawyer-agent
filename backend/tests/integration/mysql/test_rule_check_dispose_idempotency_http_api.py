...
async def _issue_status(mysql_url: URL, issue_id: str) -> str:
    engine = create_async_engine(mysql_url)
    try:
        async with engine.connect() as connection:
            value = await connection.scalar(
                text(
                    "SELECT status FROM tenant_contract_risk_issues "
                    "WHERE id = :id"
                ).bindparams(id=UUID(issue_id).bytes)
            )
            return value
    finally:
        await engine.dispose()
...
def test_rule_check_dispose_is_idempotent_over_real_mysql(
    ...
    prefix = f"lawyer-test-dispose-idem:{uuid4().hex}:"
    settings = Settings(
        environment="test",
        secret_key="h" * 32,
    ...
    with client:
        ...
        created = client.post(
            ...
            headers={
                ...
                "Idempotency-Key": "dispose-idem-tenant-1",
            },
            json={"name": "处置幂等租户", "tenant_type": "law_firm"},
        )
        ...
        # Rule pack + document + risk issue so there is something to dispose.
        pack = client.post(f"{base}/rule-packs", headers=headers, json={"name": "租赁"})
        assert pack.status_code == 201, pack.text
        pack_id = pack.json()["id"]
        assert (
            client.post(f"{base}/rule-packs/{pack_id}/activate", headers=headers).status_code
            == 200
        )
        rule = client.post(
        ...
        dispose_url = f"{base}/risk-issues/{issue_id}/disposition"
        dispose_headers = {
            "Authorization": f"Bearer {token}",
            "Idempotency-Key": "risk-issue-dispose-0000000001",
        }
        dispose_body = {"status": "accepted", "reason": "律师确认"}

        # 1. First dispose with a key succeeds.
        first = client.post(dispose_url, headers=dispose_headers, json=dispose_body)
        assert first.status_code == 200, first.text
        assert first.json()["status"] == "accepted"
        assert asyncio.run(_issue_status(migrated_mysql_url, issue_id)) == "accepted"

        # 2. Replay same key and body returns the same projection (no second write).
        replay = client.post(dispose_url, headers=dispose_headers, json=dispose_body)
        assert replay.status_code == 200, replay.text
        assert replay.json()["id"] == issue_id
        assert replay.json()["status"] == "accepted"

        # 3. Same key with a different reason conflicts.
        conflict = client.post(
            dispose_url,
            headers=dispose_headers,
            json={"status": "rejected", "reason": "冲突理由"},
        )
        assert conflict.status_code == 409
        assert conflict.json()["code"] == "idempotency_conflict"

        # 4. A fresh key on a non-open issue hits the state machine conflict.
        terminal = client.post(
            dispose_url,
            headers={
                "Authorization": f"Bearer {token}",
                "Idempotency-Key": "risk-issue-dispose-0000000002",
            },
            json={"status": "modified", "reason": "再次处置"},
        )
        assert terminal.status_code == 409
        assert terminal.json()["code"] == "rule_check_conflict"

        # 5. No-key requests still behave exactly as before (409 on non-open).
        no_key = client.post(
            dispose_url,
            headers={"Authorization": f"Bearer {token}"},
            json={"status": "rejected", "reason": "无 key 处置"},
        )
        assert no_key.status_code == 409
        assert no_key.json()["code"] == "rule_check_conflict"
