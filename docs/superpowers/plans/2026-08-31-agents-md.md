# Project Agent Instructions Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create a root `AGENTS.md` that gives every coding agent concise, durable, executable instructions for developing the enterprise Lawyer Agent safely.

**Architecture:** Use one repository-wide instruction file as the stable development constitution and link to the approved architecture specification and current implementation plans for detail. Keep volatile dependency versions and task status outside this file; retain only cross-phase product, security, evidence, engineering, and verification rules.

**Tech Stack:** Markdown, UTF-8, Python 3.12, FastAPI, pytest, Ruff, mypy, Docker Compose.

## Global Constraints

- The accepted design is `docs/superpowers/specs/2026-08-31-agents-md-design.md`.
- The file applies to the entire repository and must be stored at `AGENTS.md`.
- All text is UTF-8 and all developer commands use PowerShell-compatible syntax where platform-specific syntax is required.
- Do not duplicate the enterprise architecture specification; link to it as the source of truth.
- Do not include credentials, private legal data, temporary task state, dependency patch versions, or placeholders.
- Preserve the already approved mainland-China-only, evidence-first, multi-tenant SaaS boundaries.

## File Map

```text
AGENTS.md                                      # Repository-wide durable agent instructions
docs/superpowers/specs/2026-08-31-agents-md-design.md
                                                # Accepted design for the instruction file
docs/superpowers/specs/2026-08-31-lawyer-agent-enterprise-architecture-design.md
...
### Task 1: Repository-Wide Agent Constitution

**Files:**
- Create: `AGENTS.md`

**Interfaces:**
- Consumes: the accepted AGENTS design, the enterprise architecture specification, current repository commands, and the external legal corpus at `F:\ai律师数据库`.
- Produces: repository-wide instructions automatically discoverable by coding agents working anywhere below the repository root.

- [ ] **Step 1: Confirm the root instruction file does not already exist**
...
Expected: `False`. If it returns `True`, stop and compare the existing file with this plan instead of overwriting it.
...
foreach ($needle in $required) {
  if (-not $content.Contains($needle)) {
    throw "Missing required instruction: $needle"
  }
}
$links = @(
  'docs/superpowers/specs/2026-08-31-lawyer-agent-enterprise-architecture-design.md',
...
foreach ($path in $links) {
  if (-not (Test-Path -LiteralPath $path)) {
    throw "Missing referenced document: $path"
  }
}
if ($content -match '(?im)^\s*(TBD|TODO)\b') {
  throw 'Placeholder found in AGENTS.md'
}
'AGENTS.md contract OK'
```

Expected: `AGENTS.md contract OK`.

- [ ] **Step 4: Run existing repository quality gates**
...
Expected: all 12 current tests pass, Ruff reports `All checks passed!`, mypy reports no issues, `git diff --check` emits no errors, and Git lists only the intended new `AGENTS.md` before commit.

- [ ] **Step 5: Commit the instruction file**
