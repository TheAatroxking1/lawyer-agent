...
> 当前场景覆盖说明（2026-09-16，用户明确技术选择）：合同网页审查采用 **LlamaIndex + ReAct Agent + 版面分析文档服务 + 受控 MCP 接入**，详见[合同网页批注设计](2026-09-16-contract-web-annotations-design.md)。本场景覆盖下文原 LangChain Chain 选型与“首期不接具体 MCP 服务”的规划；MCP Gateway、租户隔离、原件不可变、证据门禁及人工复核要求继续适用。此变更不代表全部业务已迁移或依赖已安装，原规格其它场景保留原有适用范围。
