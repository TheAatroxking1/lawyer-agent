...
async def _matter_meta(mysql_url: URL, matter_id: str) -> tuple[str, str | None, str]:
    engine = create_async_engine(mysql_url)
    try:
        async with engine.connect() as connection:
            row = (
                await connection.execute(
                    text(
                        "SELECT title, description, status FROM tenant_matters "
                        "WHERE id = :id"
...
def test_matter_metadata_edits_over_real_mysql(migrated_mysql_url: URL) -> None:
    ...
    prefix = f"lawyer-test-edit:{uuid4().hex}:"
    settings = Settings(
        environment="test",
        secret_key="h" * 32,
    ...
    with client:
        ...
        for index in (1, 2):
            created = client.post(
                ...
                headers={
                    ...
                    "Idempotency-Key": f"matter-edit-tenant-create-{index}",
                },
                json={"name": f"编辑租户{index}", "tenant_type": "law_firm"},
            )
        ...
        made = client.post(
            f"{base_a}/matters",
            headers=headers_a,
            json={"title": "租赁纠纷", "kind": "litigation", "description": "初稿"},
        )
        assert made.status_code == 201, made.text
        matter_id = made.json()["id"]
        url = f"{base_a}/matters/{matter_id}"

        # 1. Patch title and description; kind/status untouched; version bumps.
        patched = client.patch(
            url,
            headers={**headers_a, "If-Match": '"1"'},
            json={"title": "房屋租赁纠纷", "description": "补充租金约定"},
        )
        assert patched.status_code == 200, patched.text
        body = patched.json()["matter"]
        assert body["title"] == "房屋租赁纠纷"
        assert body["version"] == 2
        assert body["kind"] == "litigation"
        assert body["status"] == "open"
        assert patched.headers.get("etag") == '"2"'
        stored_title, stored_desc, stored_status = asyncio.run(
            _matter_meta(migrated_mysql_url, matter_id)
        )
        assert stored_title == "房屋租赁纠纷"
        assert stored_desc == "补充租金约定"
        assert stored_status == "open"

        # 2. Description-only patch keeps the title.
        desc_only = client.patch(
            url, headers=headers_a, json={"description": "描述二版"}
        )
        assert desc_only.status_code == 200, desc_only.text
        assert desc_only.json()["matter"]["title"] == "房屋租赁纠纷"
        assert desc_only.json()["matter"]["version"] == 3

        # 3. Explicit empty description clears it.
        cleared = client.patch(url, headers=headers_a, json={"description": ""})
        assert cleared.status_code == 200, cleared.text
        assert cleared.json()["matter"]["version"] == 4
        _, stored_desc_after, _ = asyncio.run(_matter_meta(migrated_mysql_url, matter_id))
        assert stored_desc_after is None
        ...
        # 5. Blank / oversized title and empty body -> 422.
        blank = client.patch(url, headers=headers_a, json={"title": "  "})
        assert blank.status_code == 422
        empty = client.patch(url, headers=headers_a, json={})
        assert empty.status_code == 422
        oversized = client.patch(url, headers=headers_a, json={"title": "案" * 600})
        assert oversized.status_code == 422

        # 6. Missing matter -> 404.
        missing = client.patch(
            f"{base_a}/matters/00000000-0000-7000-8000-000000000042",
            headers=headers_a,
            json={"title": "不存在"},
        )
        ...
        # 7. Cross-tenant isolation (path and resource layers).
        ...
        # 8. Audit rows record each successful metadata edit.
        audit = client.get(f"{base_a}/audit?action=matter.", headers=headers_a)
        assert audit.status_code == 200, audit.text
        update_events = [
            event
            for event in audit.json()["events"]
            if event["action"] == "matter.update"
        ]
        assert len(update_events) == 3  # title+desc, desc only, clear description
        assert all(event["reason_code"] == "changed" for event in update_events)
