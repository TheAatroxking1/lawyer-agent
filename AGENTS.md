...
- Every material legal proposition must be traceable to exact evidence: document identity, article or provision, issuing authority, promulgation and effective dates, validity status, jurisdiction or region, source URL, and dataset/version metadata where available.
...
Use ordinary `docker compose down` for cleanup. Never use `down -v` unless the user explicitly authorizes deletion of named development data volumes.
